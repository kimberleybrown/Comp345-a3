	...
	if (numArm >= 0) {
		if (find(listOwned.begin(), listOwned.end(), sourceTerr) != listOwned.end()) {
			...
			for (int i = 0; i < adjList.size(); i++) {
				for (int j = 0; j < targetTerr->borders.size(); j++) {
...
Negotiate::Negotiate(Player& iPlayer, Player& targetPlayer) {
	...
	this->effect = new string(" A negotiate order targets an enemy player. It results in the target player and the player issuing the order to not be able to successfully attack each others� territories for the remainder of the turn.");
	this->iPlayer = &iPlayer;
