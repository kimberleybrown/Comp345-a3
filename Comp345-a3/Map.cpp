...
Map::Map(MapLoader* mapLoaderObject)
{
    ...
    for (int i = 0; i < continentCount; i++) {
        ...
        cout << "assigning contVector armies = " << contVector.at(i).armyCount << endl;
...
//    this->continentsArray = new Continent[continentCount];
    this->contVector.resize(continentCount);

    // loop to create continent linked lists within Continent array
    for (int i = 0; i < continentCount; i++) {
        this->contVector[i].head = nullptr;
        this->contVector[i].length = 0;
    }
}
...
//    this->territoryStartIndex = map.territoryStartIndex;
    ...
    for (int i = 0; i < territoryCount; i++) {
        ...
        Edge* temp_edge = map.terrVector[i].head;
